  ...
  const loadFile = async (path: string) => {
    ...
    try {
      const res = await fetch(
        `/api/github/file?branch=${selectedBranch}&path=${encodeURIComponent(path)}`
      );
      const json = await res.json();
      if (json.status === "success") {
        ...
        <div className="col-span-12 lg:col-span-7 bg-[#111111] border border-white/5 rounded-2xl flex flex-col overflow-hidden shadow-xl">
          <div className="p-4 border-b border-white/5 bg-black/20 flex items-center justify-between">
            ...
            {selectedFileContent && (
              <button
                onClick={importFile}
                className="bg-violet-600 hover:bg-violet-500 text-white px-3 py-1 rounded text-xs font-medium transition-colors flex items-center gap-1.5"
              >
            ...
            {loadingFile ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-3">
                ...
                <span className="text-sm">
                  Select a file from the tree to preview and import.
                </span>
              </div>
            )}
          </div>
