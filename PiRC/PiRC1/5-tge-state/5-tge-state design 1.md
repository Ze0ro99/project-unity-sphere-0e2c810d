...
If (theoretical worst case) _all_ of that $T_{out}$ is eventually market-sold into the LP, then the pool’s token reserve becomes $y_{min} = T + T_{out} = 2T + T_{engage}$, and by the invariant the Pi reserve becomes:
...
In the base case with no rewards ($T_{engage}=0$), this gives $p_{floor}=0.25\,p_{list}$. With the rewards parameter from Section 4 ($T_{engage}=5\%T$), the **lower bound on the LP’s _spot price_ (Pi per token) is $p_{floor}\approx 0.238\,p_{list}$**. There is no higher bound.
