...
- **Step 3 Automated Engagement-Based Swaps:**
  ...
  ***
  - The swaps are execuated based on Engagement Score rank: the highest-engaged users swap first at the initial spot price of the pool ($ p*{init} = p*{list}/4$), getting highest discount; Less-engaged users will swap later at incrementally less discounted prices, up to a maximum price that equals undiscounted the listing price, $p\_{last}.
...
However, each participant’s **effective acquisition price** uses _both_ buckets:
